...
void clear_shared_memory(shm_xy *p) {
  /*
   * Clear shared memory safely using the same seq protocol.
   *
   * Odd seq  = write/reset in progress
   * Even seq = stable completed write/reset
   */

  p->seq = 1;              // reset in progress
  ...
  p->seq = 2;              // reset complete
}

bool read_stable_message(
    const shm_xy *p,
    uint32_t &last_seq,
    uint32_t &kind,
    double &x_mm,
...
int main() {
  ...
  if (!chip0) {
    fprintf(
        stderr,
        "Failed to open %s for motor pins: %s\n",
        gpio_chip0_char,
        strerror(errno)
    );
  ...
  if (!chip4) {
    fprintf(
        stderr,
        "Failed to open %s for limit switches: %s\n",
        gpio_chip4_char,
        strerror(errno)
    );
  ...
  /*
   * Clear any stale coordinate left by a previous Python/C++ run
   * before C++ starts reading.
   */
  clear_shared_memory(p_shm);

  std::cout << "Waiting for coordinates from Python...\n";

  /*
   * Start from 2 because clear_shared_memory() ends with seq = 2.
   * This prevents C++ from treating the cleared packet as a new coordinate.
   */
  uint32_t last_seq = 2;
  ...
  while (keep_running) {
    ...
    if (kind == KIND_TOP_SIDE || kind == KIND_NO_TRACK) {
      try {
        ...
        std::cerr << "[C++] move to rest point failed: "
                  << e.what() << "\n";
      }
