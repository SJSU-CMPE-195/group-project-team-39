...
ROI_X, ROI_Y, ROI_W, ROI_H = 213, 1, 617, 719  # Original crop from raw camera frame
...
def px_to_mm(pt):
    ...
    x_mm = pt[0] * MM_PER_PX_X
    y_mm = (PROC_H - 1 - pt[1]) * MM_PER_PX_Y
...
# ROI_X, ROI_Y, ROI_W, ROI_H = 69, 23, 521, 418  # Original crop from raw camera frame

# # Rotate cropped ROI 90 degrees counterclockwise
# ROTATE_CAMERA_CCW = True

# # Processed frame size after crop/rotation
# if ROTATE_CAMERA_CCW:
#     PROC_W = ROI_H
#     PROC_H = ROI_W
# else:
#     PROC_W = ROI_W
...
#     # Then rotate the cropped ROI 90 degrees counterclockwise.
#     # All x/y coordinates after this point use the rotated frame.
#     if ROTATE_CAMERA_CCW:
...
# print(f"[INFO] Camera source resolution: {actual_w}x{actual_h}", flush=True)
# print(f"[INFO] Raw ROI: x={ROI_X} y={ROI_Y} w={ROI_W} h={ROI_H}", flush=True)
# print(f"[INFO] Rotate CCW: {ROTATE_CAMERA_CCW}", flush=True)
# print(f"[INFO] Processed frame resolution: {PROC_W}x{PROC_H}", flush=True)
...
#         # Crop ROI, then rotate 90 degrees counterclockwise.
#         # Every coordinate after this is in the rotated frame.
#         frame = preprocess_frame(raw_frame)
#         h, w = frame.shape[:2]
...
#                 )
#                 print(
#                     f"[PY->C++] kind=NO_TRACK x=0 y=0 x_mm=0.0 y_mm=0.0 "
#                     f"send_time_ms={dt_ms:.3f}",
#                     flush=True
#                 )
...
# # ROI_X, ROI_Y, ROI_W, ROI_H = 69, 23, 521, 418 # (521 x 418) res
...
# # def preprocess_frame(frame):
# #     # Crop enabled: use the selected ROI from the full 640x480 frame
...
# # # Fixed pixel-to-mm conversion for cropped frame
# # MM_PER_PX_X = TABLE_W_MM / ROI_W
# # MM_PER_PX_Y = TABLE_H_MM / ROI_H
...
# #     # using bottom-left as the physical origin.
# #     if pt is None:
# #         return None
...
# #                     # Bottom-left origin: left/right walls can bounce
# #                     if hit_wall in ("left", "right"):
# #                         v_out = reflect((ux, uy), hit_wall)
...
# #                     print(
# #                         f"[PY->C++] kind={predicted_label} "
# #                         f"x={predicted_pt[0]} y={ROI_H - 1 - predicted_pt[1]} "
# #                         f"x_mm={predicted_mm[0]:.1f} y_mm={predicted_mm[1]:.1f} "
# #                         f"send_time_ms={dt_ms:.3f}",
# #                         flush=True
# #                     )
